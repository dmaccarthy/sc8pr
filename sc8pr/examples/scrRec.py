...
from sc8pr.misc.media import Grabber, ImageIO
...
class SaveThread(Thread):
    ...
    def run(self):
        if self.output == "s8v" or not hasattr(ImageIO, "im"):
            self.s8v(self.fn, self.frames, self.fps, self.frameTimes)
        else:
            with imageio.get_writer(self.fn, fps=self.fps) as writer:
