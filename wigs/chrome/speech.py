...
from wigs.util import wigsPath
...
class Server(Thread):
    ...
    def wsgi(env, start_response):
        port = int(env["HTTP_HOST"].split(":")[1])
        s = Server.servers[port]
        try:
        ...
        if n == 0:
            ...
            if n == 2:
                data = {}
                s.voices = unquote(env["QUERY_STRING"]).split("&")
            else:
                data = s.queue
                s.queue = []
                n = len(data)
    ...
    def run(self):
        ...
        while not self.kill or len(self.queue):
            self.srv.handle_request()
        print(msg.format("Closing"), file=stderr)
    ...
    def stop(self): self.kill = True

    with open(wigsPath("chrome/index.html")) as f: html = f.read()
